...
void hyperion_ui_init(void)
{
	...
	ui.dispmode = HYPERION_UI_DISPMODE_BOOT;
...
void hyperion_ui_render_boot(hyperion_ui_t * ui)
{
	...
	hyperion_ui_text(ui, 0, 0, ui->w, 0, justifyCenter, "hyperion-bl");
	y = 16;

	chsnprintf(buf, sizeof(buf) - 1, "%08x/%d", HYPERION_HW_SW_VER, hyperion_can.node_id);
