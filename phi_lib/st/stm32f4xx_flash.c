...
void FLASH_SetLatency(uint32_t FLASH_Latency)
{
  ...
  chDbgCheck(IS_FLASH_LATENCY(FLASH_Latency));
...
{
  /* Check the parameters */
  chDbgCheck(IS_FUNCTIONAL_STATE(NewState));
...
FLASH_Status FLASH_EraseSector(uint32_t FLASH_Sector, uint8_t VoltageRange)
{
  ...
  /* Check the parameters */
  chDbgCheck(IS_FLASH_SECTOR(FLASH_Sector));
  chDbgCheck(IS_VOLTAGERANGE(VoltageRange));
...
FLASH_Status FLASH_ProgramDoubleWord(uint32_t Address, uint64_t Data)
{
  ...
  /* Check the parameters */
  chDbgCheck(IS_FLASH_ADDRESS(Address));
...
{  
  ...
  /* Check the parameters */
  chDbgCheck(IS_OB_PCROP_SELECT(OB_PcROP));
...
{ 
  ...
  /* Check the parameters */
  chDbgCheck(IS_OB_PCROP(OB_PCROP));
  chDbgCheck(IS_FUNCTIONAL_STATE(NewState));
...
void FLASH_OB_UserConfig(uint8_t OB_IWDG, uint8_t OB_STOP, uint8_t OB_STDBY)
{
  ...
  /* Check the parameters */
  chDbgCheck(IS_OB_IWDG_SOURCE(OB_IWDG));
  chDbgCheck(IS_OB_STOP_SOURCE(OB_STOP));
  chDbgCheck(IS_OB_STDBY_SOURCE(OB_STDBY));
...
{
  /* Check the parameters */
  chDbgCheck(IS_FLASH_IT(FLASH_IT)); 
  chDbgCheck(IS_FUNCTIONAL_STATE(NewState));
...
FlagStatus FLASH_GetFlagStatus(uint32_t FLASH_FLAG)
{
  ...
  /* Check the parameters */
  chDbgCheck(IS_FLASH_GET_FLAG(FLASH_FLAG));
...
{
  /* Check the parameters */
  chDbgCheck(IS_FLASH_CLEAR_FLAG(FLASH_FLAG));
